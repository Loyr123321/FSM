...
builder.Services.AddAuthentication(CookieAuthenticationDefaults.AuthenticationScheme /*JwtBearerDefaults.AuthenticationScheme*/)
    ...
    .AddJwtBearer(options =>
    {
        ...
        options.Events.OnChallenge = context =>
        {
            ...
            {
                string responce = JsonConvert.SerializeObject(new BaseMobileResponse(null, (int)ErrorHandling.ErrorCodes.ERROR_TOKEN_HAS_EXPIRED), settings);
            ...
            {
                string responce = JsonConvert.SerializeObject(new BaseMobileResponse(null, (int)ErrorHandling.ErrorCodes.ERROR_TOKEN_IS_NOT_WHITELISTED), settings);
                return context.Response.WriteAsync(responce);
            }

            return context.Response.WriteAsync(JsonConvert.SerializeObject(new BaseMobileResponse(null, null).ToJson(), settings));
        };
    });

builder.Services.AddAuthorization(options =>
{
    options.AddPolicy("RequireRole", policy => policy.RequireRole(Utils.ROLES)); //Require_Role требовать роль
});

//builder.Services.AddScoped<StateContainer>(); //STATE_CONTAINER
builder.Services.AddScoped<ICookie, BlazorApp1.Utils.Cookie>();
...
//Solve Mongo DB Element Does Not Match Any Field or State of Class (Разрешить несоответствие полей в бд и в моделях)
...
app.MapGet("/loginUser", async (HttpContext context, string token) =>
{
    ...
    if (isValid == false) 
    {
        context.Response.Redirect("/login?errorCode="+ (int)ErrorHandling.ErrorCodes.ERROR_INVALID_TOKEN, permanent: false);
        return "";
    }
    ...
    if (tokenUser == null) 
    {
        context.Response.Redirect("/login?errorCode=" + (int)ErrorHandling.ErrorCodes.ERROR_USER_NOT_FOUND, permanent: false);
        return "";
    }
    //Пользователь заблокирован
    if (tokenUser.IsActive == false)
    {
        context.Response.Redirect("/login?errorCode=" + (int)ErrorHandling.ErrorCodes.ERROR_USER_BLOCKED, permanent: false);
        return "";
    }
...
app.MapGet("/test1", async (HttpContext context) =>
{
    //await context.
    //context.Response.Redirect("/", permanent: false);

    var message = context.Request.PathBase;
