...
namespace BlazorApp1.Utils
{
    public class ExcelUtils
    {
        ...
        //orderRow(Последний заказ выбранного шаблона) для получения полей и заголовков
        public static string OrdersNew(string sheetName, OrderGridView orderRow, List<OrderGridView> values)
        {
            try
            {
                HSSFWorkbook hssfworkbook = new HSSFWorkbook();
                ...
                var headers = orderRow.AllFields;
                //Заголовки 0-строка
                IRow row = sheet.CreateRow(0);
                for (int i = 0; i < headers.Count; i++)
                {
                ...
                {
                    row = sheet.CreateRow(i); //1я и тд строки
                    for (int j = 0; j < values[i].AllFields.Count; j++)
                    {
                        ICell cell = row.CreateCell(j);
                        cell.SetCellValue(values[i].AllFields[j].FieldValueAsString);

                        cell.CellStyle = myStyle;
                        cell.CellStyle.SetFont(font);

                        //sheet.SetColumnWidth(j, GetLenght(values[i].AllFields[j].FieldValueAsString) * 256);
                    }
                    //sheet.AutoSizeColumn(i); //!!!Автоподбор
